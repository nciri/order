package com.buyden.order.web.rest;

import com.buyden.order.config.KafkaProperties;
...
class OrderKafkaResourceIT {
    ...
    void setup() {
        ...
        OrderKafkaResource kafkaResource = new OrderKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/order-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/order-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
